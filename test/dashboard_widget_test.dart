...
void main() {
  ...
  testWidgets('Should display the transfer feature when Dashboard is opened',
    ...
    final transferMiniCard = find.byWidgetPredicate((widget) => findMiniCard(widget, 'Transfer', Icons.monetization_on));
  ...
  testWidgets('Should display the feed feature when Dashboard is opened',
          (WidgetTester tester) async {
        await tester.pumpWidget(MaterialApp(home: Dashboard(contactDao: MockContactDao())));
        final feedMiniCard = find.byWidgetPredicate((widget) => findMiniCard(widget, 'Feed', Icons.description));
